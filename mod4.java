...
// starting point at green 
Cell current = stack.peek();
// testing alll the direction and if we hit and line/ wall move 
...
else if(!current.eastWall && !visited(current.row, 
current.col+1)){
stack.push(maze[current.row][current.col+1]);
current.setBackground(Color.yellow);
}
else if(!current.westWall && !visited(current.row, current.col-
1)){
stack.push(maze[current.row][current.col-1]);
current.setBackground(Color.yellow);
}
else if(!current.southWall && !visited(current.row+1, 
current.col)){
stack.push(maze[current.row+1][current.col]);
current.setBackground(Color.yellow);
}
else {
// set the background to pink  when our route is not right
current.setBackground(Color.pink);
stack.pop();
}
}
// set our starting mark as green 
